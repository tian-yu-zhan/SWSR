...
# setwd("~/Dropbox/Research/AbbVie/Time_drfit/Final_v3/TD_8_enroll/")
# n.cluster = 8
...
for (scen.ind in c(1:8)){
  ...
  p.vec = foreach(itt = 1:n.itt) %dopar% {
    ...
    data.temp = data.frame("ind" = runif(n, min = 1, max = n),
                           "grp" = 1)
    ...
    if (method.ind==1){
        ...
        for (cross.feature.itt in 1:length(bs.knots.cand)){
          ...
          for (cross.itt in 1:n.cross){
            ...
            rlm.fit.init = lm(x~grp+bs(ind, knots = 
                                         as.numeric(round(quantile(data.cross.all$ind, 
                            prob = (1:bs.knots.temp.in)/(bs.knots.temp.in+1)))),
      ...
      rlm.fit.init = lm(x~grp+bs(ind, knots = 
                                   as.numeric(round(quantile(data.in$ind, prob = (1:bs.knots.final)/(bs.knots.final+1)))),                                  
...
write.csv(cbind(table.name.out, table.out), 
          paste0("Enroll_time_drift_sd1_", sd.1, "_sd2_", sd.2, 
                 "_trtprop_", trt.prop, 
                 ".csv", sep=""))
